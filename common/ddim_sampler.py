...
from common.utils_diff import get_beta_schedule
...
def alpha_prod(beta, t):
    beta = torch.cat([torch.zeros(1).to(beta.device), beta], dim=0)
    a = (1 - beta).cumprod(dim=0).index_select(0, t + 1).view(-1, 1, 1)
    return a

def ddim_sampler(x, src_mask, model, seq, beta_schedule):
    with torch.no_grad():
        n = x.size(0)
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            # The t th step of alpha, as in DDIM paper, Equation 12 
            at = alpha_prod(beta_schedule, t.long())
            at_pred = alpha_prod(beta_schedule, next_t.long()) 
            xt = xs[-1]
            et = model(xt, src_mask, t.float(), 0) 
            #sigma_t = (kwargs.get("eta", 0) * ((1 - at / at_pred) * (1 - at_pred) / (1 - at)).sqrt())
            sigma_t = 0
            c_direction = (1 - at_pred - sigma_t ** 2).sqrt()
            x0_pred = (xt - et * (1 - at).sqrt())/ at.sqrt()
            xt_next = at_pred.sqrt() * x0_pred \
                  + sigma_t * torch.randn_like(x) + c_direction * et
            x0_samples.append(x0_pred)
            xs.append(xt_next)
    return xs, x0_samples
...
class Diffpose(object):
    ...
    def test_hyber(self, is_train=False):
        ...
        for i, (_, input_noise_scale, input_2d, targets_3d, input_action, camera_para) in enumerate(data_loader):
            ...
            output_uvxyz = ddim_sampler(x, src_mask, seq, self.model_diff, self.betas)
